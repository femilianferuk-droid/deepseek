        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                if textarea.is_visible():
                    ...
                    # Сохраняем скриншот после ответа
                    page.screenshot(path='debug_after.png')
                    logger.info("Скриншот после ответа сохранен: debug_after.png")
                    
                    # Логируем весь текст страницы
                    body_text = page.locator('body').inner_text()
                    logger.info(f"Текст страницы: {body_text[:500]}...")
                            ...
                            for i, elem in enumerate(elements):
                                try:
                                    ...
                                    if text and len(text) > 20:
                                        logger.info(f"Найден текст в элементе {i}: {text[:100]}...")
                                        response_text = text
                                except:
                                    pass
                        except Exception as e:
                            logger.error(f"Ошибка при поиске селектора {selector}: {e}")
                    
                    if response_text:
                        logger.info(f"Ответ получен: {response_text[:200]}...")
                        return response_text
                    else:
                        logger.warning("Ответ не найден!")
                        return "Не удалось получить ответ от DeepSeek"
                else:
                    logger.error("Поле ввода не найдено!")
                    return "Поле ввода не найдено"
...
async function sendMessage() {
    ...
    try {
        ...
        const messages = container.getElementsByClassName('message');
        const lastMessage = messages[messages.length - 1];
        lastMessage.querySelector('.message-content').textContent = 'Ошибка: ' + e.message;
    }
...
def send_message():
    try:
        ...
        logger.info("Отправка в DeepSeek...")
        response = deepseek.send_message(message)
        logger.info(f"Ответ от DeepSeek: {response[:100]}...")
        ...
        return jsonify({
            ...
            'assistant_message': f'Ошибка: {str(e)}'
        })
